...
import { ChartLine, TrendingUp, TrendingDown, ArrowRight } from "lucide-react";
...
export const DownloadLytics = () => {
  ...
  const totalDownloads = downloadData.reduce((acc, item) => acc + item.downloads, 0);
  const averageDownloads = Math.round(totalDownloads / downloadData.length);
  const lastDayDownloads = downloadData[downloadData.length - 1]?.downloads || 0;
  const previousDayDownloads = downloadData[downloadData.length - 2]?.downloads || 0;
  const downloadTrend = ((lastDayDownloads - previousDayDownloads) / previousDayDownloads) * 100;
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="shadow-none">
          <CardContent className="p-4">
            ...
            <p className="mt-2 text-2xl font-bold">{millify(averageDownloads)}</p>
            <p className="mt-1 text-xs text-muted-foreground">downloads per day</p>
          </CardContent>
        </Card>
        ...
        <Card className="shadow-none">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <p className="text-sm font-medium text-muted-foreground">Latest Downloads</p>
              <ArrowRight className="h-4 w-4 text-muted-foreground" />
            </div>
            <p className="mt-2 text-2xl font-bold">{millify(lastDayDownloads)}</p>
            <p className="mt-1 text-xs text-muted-foreground">
              on {dayjs(downloadData[downloadData.length - 1]?.day).format("MMM D, YYYY")}
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="shadow-none">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <p className="text-sm font-medium text-muted-foreground">24h Trend</p>
              {downloadTrend > 0 ? (
            ...
            <p className="mt-1 text-xs text-muted-foreground">compared to previous day</p>
          </CardContent>
        </Card>
      </div>
